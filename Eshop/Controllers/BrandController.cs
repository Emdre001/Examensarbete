...
namespace AppWebApi.Controllers
{
    ...
    public class BrandController : Controller
    {
        ...
        [Authorize(AuthenticationSchemes = Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerDefaults.AuthenticationScheme,
            Policy = null, Roles = "supusr, sysadmin")]
        [HttpGet()]
        [ProducesResponseType(200, Type = typeof(ResponseItemDto<BrandCuDto>))]
        [ProducesResponseType(400, Type = typeof(string))]
        [ProducesResponseType(404, Type = typeof(string))]
        public async Task<IActionResult> ReadItemDto(string id = null)
        {
            try
            {
                ...
                return Ok(
                    new ResponseItemDto<BrandCuDto>() {
                    DbConnectionKeyUsed = item.DbConnectionKeyUsed,
                    Item = new BrandCuDto(item.Item)
                });
            }
            catch (Exception ex)
            {
        ...
        [ProducesResponseType(400, Type = typeof(string))]
        public async Task<IActionResult> UpdateItem(string id, [FromBody] BrandCuDto item)
        {
            try
            {
                var idArg = Guid.Parse(id);
        ...
        [ProducesResponseType(200, Type = typeof(ResponseItemDto<IBrand>))]
        [ProducesResponseType(400, Type = typeof(string))]
        public async Task<IActionResult> CreateItem([FromBody] BrandCuDto item)
        {
            try
            {
